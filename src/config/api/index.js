  ...
  get: (path) => {
    ...
    if (typeof id === 'undefined') {
      if (path.match(/lists/g)) {
        ...
        let todos = (status&&status!='')?
          (fakeDB.todos.filter(todo.completed == (status=='complete'))):
          (fakeDB.todos)
        data = {todos: todos}
      }
    } else {
      if (path.match(/lists/g)) {
        ...
        data = {
          list: {
            ...list,
            todos: (status&&status!='')?
              (fakeDB.todos.filter(todo => todo.listID === list.id && (todo.completed == (status=='complete')))):
              (fakeDB.todos.filter(todo => todo.listID === list.id))
          }
        }
      } else {
